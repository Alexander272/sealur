...
func (s *DataService) getDataFlange(
	...
	typeFlange string,
	temp float64,
) (flangeData *cap_model.FlangeResultOld, boltSize *cap_model.BoltResult, err error) {
	if flange.StandartId == "another" {
		flangeData = &cap_model.FlangeResultOld{
...
func (s *DataService) getCalculatedDataFlange(
	...
	data *cap_model.FlangeResultOld,
	Dcp float64,
) (*cap_model.FlangeResultOld, error) {
	calculated := data
