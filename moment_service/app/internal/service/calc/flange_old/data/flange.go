...
func (s *DataService) getDataFlange(
	...
	typeFlange string,
	temp float64,
) (flangeData *flange_model.OldFlangeResult, boltSize *flange_model.BoltResult, err error) {
	if fl.StandartId == "another" {
		flangeData = &flange_model.OldFlangeResult{
			DOut: fl.Size.DOut,
			D:    fl.Size.D,
			Dk:   fl.Size.Dk,
			Dnk:  fl.Size.Dnk,
	...
	if fl.Type == flange_model.FlangeData_free {
		...
		if fl.RingMarkId != "another" {
			var err error
			mat, err = s.materials.GetMatForCalculate(ctx, fl.RingMarkId, flangeData.Tk)
			if err != nil {
				return nil, nil, err
			}
		} else {
...
func (s *DataService) getCalculatedDataFlange(
	...
	Dcp float64,
) (*flange_model.OldFlangeResult, error) {
	calculated := data
