...
func (r *AdditRepo) GetAll() (addit []*proto.Additional, err error) {
	query := fmt.Sprintf("SELECT materials, mod, temperature, mounting FROM %s", AdditionalTable)
...
func (r *AdditRepo) Create(add *proto.CreateAddRequest) error {
	query := fmt.Sprintf("INSERT INTO %s (materials, mod, temperature, mounting) VALUES ($1, $2, $3, $4)", AdditionalTable)

	_, err := r.db.Exec(query, add.Materials, add.Mod, add.Temperature, add.Mounting)
	if err != nil {
