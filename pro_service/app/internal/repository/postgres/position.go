...
func (r *PositionRepo) Create(ctx context.Context, position *position_model.FullPosition) (string, error) {
	...
	_, err := r.db.Exec(query, id, position.OrderId, position.Title, position.Amount, position.Type.String(), position.Count, position.Info)
	if err != nil {
		return "", fmt.Errorf("failed to execute query. error: %w", err)
	}
	return id.String(), nil
...
func (r *PositionRepo) CreateSeveral(ctx context.Context, positions []*position_model.FullPosition) error {
	query := fmt.Sprintf("INSERT INTO %s (id, order_id, title, amount, type, count, info) VALUES ", PositionTable)
	...
	for i, s := range positions {
		...
		args = append(args, s.Id, s.OrderId, s.Title, s.Amount, s.Type.String(), s.Count, s.Info)
	}
...
func (r *PositionRepo) Update(ctx context.Context, position *position_model.FullPosition) error {
	...
	_, err := r.db.Exec(query, position.Title, position.Amount, position.Info, position.Id)
	if err != nil {
		return fmt.Errorf("failed to execute query. error: %w", err)
	}
	return nil
