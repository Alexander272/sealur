...
type TokenManager interface {
	NewJWT(userId, role string, ttl time.Duration) (time.Time, string, error)
...
func (m *Manager) NewJWT(userId, role string, ttl time.Duration) (iat time.Time, token string, err error) {
	iat = time.Now()
