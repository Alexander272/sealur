...
type Handler struct {
	...
	ringsKitSizeApi rings_kit_size_api.RingSizesKitServiceClient
...
func (h *Handler) InitRoutes(conf config.ServicesConfig, api *gin.RouterGroup) {
	...
	h.ringsKitSizeApi = rings_kit_size_api.NewRingSizesKitServiceClient(connect)
	...
	h.initRingMaterialsRoutes(pro)
	h.initRingModifyingRoutes(pro)
	h.initRingSizeRoutes(pro)
}
