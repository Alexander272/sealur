...
func (h *Handler) Init(conf *config.Config, group *gin.RouterGroup) {
	botApi := api.NewMostApi(conf.Services.MostBot.Url)
	...
	userHandler := user.NewHandler(conf, botApi, h.services, h.middleware, CookieName)
	...
	new_pro.NewHandler(botApi, h.middleware).InitRoutes(conf.Services, v1)
}
